...
contract MultiSig{
    ...
    event tokenTransferred(address sender, address reciever, uint256 amount);
    ...
    modifier allowanceValid(uint amount, address sender){
        ERC20Mintable erc20Mintable = ERC20Mintable(assetContract);
        uint allowedAmount = erc20Mintable.allowance(sender,address(this));
        require(amount <= allowedAmount,"Trying to spend more than allowed");
        _;
    }
    ...
    modifier transferValid(uint amount, address sender){
        ERC20Mintable erc20Mintable = ERC20Mintable(assetContract);
        uint allowedAmount = erc20Mintable.balanceOf(sender);
        require(amount <= allowedAmount,"Trying to send more than available");
        _;
    }
    ...
    function deposit(uint256 amount) public onlyParticipant allowanceValid(amount,msg.sender) {
    ...
    function depositWithSignature(uint256 amount, bytes memory signedDepositObject) public{
        
        bytes memory expectedFormat = abi.encodePacked(amount, address(this));
        ...
        address signer = recoverSigner(message, signedDepositObject);
        ERC20Mintable erc20Mintable = ERC20Mintable(assetContract);
        uint allowedAmount = erc20Mintable.allowance(signer,address(this));
        require(amount <= allowedAmount,"Trying to spend more than allowed");
        balances[signer] += amount;
        totalBalance += amount;
        emit NewDeposit(amount, signer);
        bool success = erc20Mintable.transferFrom(signer,address(this),amount);
        require(success,");
        balances[signer] += amount;
        totalBalance += amount;
        emit NewDeposit(amount, signer);
    ...
    //for custodial transfer of tokens
    function transferToken(uint amount,address reciever) public 
    allowanceValid(amount,msg.sender)
    transferValid(amount,msg.sender)
    {
        ERC20Mintable erc20Mintable = ERC20Mintable(assetContract);
       bool success = erc20Mintable.transferFrom(msg.sender,reciever,amount);
        require(success);
        emit tokenTransferred(msg.sender,reciever,amount);
